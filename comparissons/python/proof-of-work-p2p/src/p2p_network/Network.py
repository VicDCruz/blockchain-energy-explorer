...
from src.p2p_network import commands
from src.p2p_network.Message import Message
from src.p2p_network.Node import Node
from src.p2p_network.helpers.terminal_helper import print_colored
...
class Network(Node):
    ...
    def __init__(self, ip="", port=None):
        self.id = hashlib.sha256(f"{random.randint(0, 99999)}".encode()).hexdigest()
        ...
        while connected:
            ...
            if (msg_len):
                ...
                if (commands.NODE_CON_ADDR in msg['title']):  # CONNECT TO ME COMMAND
                    ...
                    if conn_index == -1:
                        self.connectToNode(conn_ip,
                                           int(conn_port))  # if connection has not established so far, connect to the node
                ...
                if "#BROADCAST" == msg["title"]:
                    ...
                    self.messages_received.put(msg['message'])
                    print("TMP QUEUE SIZE: " + str(self.messages_received.qsize()))

                    if self.messages_received.qsize() >= 5:
                        print("TMP QUEUE SIZE: " + str(self.messages_received.qsize()))
                        connected = False
                    ...
                    print("Income message from " + msg["message"])
                    self.messages_received.put(msg)
                    print("TMP QUEUE SIZE: " + str(self.messages_received.qsize()))
    ...
    def connectToNode(self, address, port):
        print(f"_______{address}_{port}_______")
